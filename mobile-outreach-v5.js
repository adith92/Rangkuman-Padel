...
function venueShell(){return `
 <div class="venue-mobile-head">
  <div><span class="report-kicker">MOBILE OUTREACH DIRECTORY</span><h2>Venue Padel & Kontak Langsung</h2><p>Kontak venue dikumpulkan dan diverifikasi oleh tim dari AYO, situs resmi, Instagram resmi, Google Maps, dan kanal publik pengelola.</p></div>
 </div>
 ...
 <div class="venue-source-note"><strong>Standar kontak:</strong> pengguna tidak diminta mencari sendiri. Tombol WhatsApp, telepon, email, atau DM hanya muncul setelah kontak cocok dengan sumber publik venue.</div>`}
...
function visualHtml(venue,meta){
 ...
 return `<div class="venue-court-art" aria-label="Foto ${esc(venue.name)} belum tersedia"><i></i><span>${esc(initials(venue.name))}</span></div>`;
}
...
function directActions(venue,meta){
 ...
 const message=formalMessage(venue.name);
 const actions=[];
 if(whatsapp)actions.push(`<a class="venue-action primary wa" href="https://wa.me/${whatsapp}?text=${encodeURIComponent(message)}" target="_blank" rel="noopener"><span>💬</span><b>WhatsApp</b></a>`);
 ...
 if(email)actions.push(`<a class="venue-action email" href="mailto:${encodeURIComponent(email)}?subject=${encodeURIComponent(EMAIL_SUBJECT)}&body=${encodeURIComponent(message)}"><span>📧</span><b>Email</b></a>`);
 ...
 actions.push(`<a class="venue-action ayo" href="${ayoUrl(venue)}" target="_blank" rel="noopener"><span>🎾</span><b>Buka AYO</b></a>`);
 return actions.join('');
}
...
function contactRows(meta){
 ...
 return [
  ...
  meta.phone&&`<div><span>Telepon</span><strong>${esc(meta.phoneLabel||meta.phone)}</strong></div>`,
...
function renderVenueCard(venue){
 ...
 return `<article class="venue-card venue-card-v5 ${verified?'verified':'pending'}">
  <div class="venue-thumb venue-thumb-v5">${visualHtml(venue,meta)}<span class="venue-city">${esc(venue.city)} • ${esc(venue.province)}</span><span class="venue-data-badge ${verified?'verified':'pending'}">${verified?'DATA KONTAK ADA':'VERIFIKASI KONTAK'}</span></div>
  <div class="venue-body venue-body-v5">
   <div class="venue-title-row"><div><h3>${esc(venue.name)}</h3><p class="venue-location">${esc(venue.city)}, ${esc(venue.province)}</p></div><span class="venue-source-mini">${esc(meta.updated||'Sedang diverifikasi')}</span></div>
   ...
   ${contactInfo?`<div class="venue-contact-grid">${contactInfo}</div>`:`<div class="venue-no-contact"><strong>Kontak sedang diverifikasi oleh tim.</strong><span>Nomor, WhatsApp, email, atau Instagram akan ditampilkan setelah cocok dengan sumber publik resmi.</span></div>`}
   ${previewHtml(venue,meta)}
   <div class="venue-actions venue-actions-v5">${directActions(venue,meta)}</div>
   <div class="venue-source-row"><span>🛡️ ${esc(meta.source||'Sumber kontak sedang diverifikasi')}</span>${meta.sourceUrl?`<a href="${esc(meta.sourceUrl)}" target="_blank" rel="noopener">Buka sumber</a>`:''}</div>
  </div>
 </article>`;
}
...
function waNumberFromHref(href=''){
 try{return cleanNumber(new URL(href,location.href).pathname)}catch{return cleanNumber(href.split('?')[0])}
}
...
function enhanceContactCard(card){
 ...
 const message=formalMessage(name);
 const wa=card.querySelector('a.contact-action.wa');
 if(wa){const number=waNumberFromHref(wa.href);wa.href=`https://wa.me/${number}?text=${encodeURIComponent(message)}`;wa.querySelector('b').textContent='WhatsApp'}
 ...
 if(email){const address=email.getAttribute('href').replace(/^mailto:/i,'').split('?')[0];email.href=`mailto:${encodeURIComponent(address)}?subject=${encodeURIComponent(EMAIL_SUBJECT)}&body=${encodeURIComponent(message)}`}
 card.querySelectorAll('.contact-copy-v5,.copy-salam').forEach(item=>item.remove());
}
