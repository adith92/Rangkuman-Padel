...
function getVenues(){
 const base=(typeof NATIONAL_POINTS!=='undefined'&&Array.isArray(NATIONAL_POINTS)?NATIONAL_POINTS:[]).filter(point=>point.category==='padel');
 const extra=(Array.isArray(window.PBPI_COURT_RESEARCH)?window.PBPI_COURT_RESEARCH:[]).filter(row=>!row.is_existing_venue).map(row=>({
  name:row.display_name||row.canonical_name,
  category:'padel',
  city:row.city,
  province:row.province,
  address:row.address,
  precision:'name_search',
  searchQuery:`${row.canonical_name}, ${row.address}, Indonesia`,
  researchVenueId:row.venue_id
 }));
...
function metaFor(venue){
 return (window.VERIFIED_VENUE_CONTACTS&&window.VERIFIED_VENUE_CONTACTS[venue.name])||{};
}
...
function shell(){return `
 <div class="venue-mobile-head">
  <div><span class="report-kicker">DIREKTORI VENUE & RISET COURT</span><h2>Venue Padel, Jumlah Court & Kontak</h2><p>Direktori nasional digabung dengan riset Jabodetabek terbaru. Kontak, jumlah court, konflik, dan status verifikasi dipisahkan agar tidak ada angka yang menyamar sebagai kepastian.</p></div>
 </div>
 <div class="venue-summary">
  <div><b id="venueTotal">0</b><span>Total venue</span></div>
  <div><b id="venueAddress">0</b><span>Alamat / pencarian</span></div>
  <div><b id="venueContact">0</b><span>Kontak langsung</span></div>
  <div><b id="venueSocial">0</b><span>Instagram</span></div>
 </div>
 <div class="venue-toolbar venue-toolbar-v5">
  <label><span>Cari venue</span><input id="venueSearch" type="search" inputmode="search" placeholder="Nama venue, kota, ID PBPI..."></label>
  <label><span>Provinsi</span><select id="venueProvince"><option value="">Semua provinsi</option></select></label>
  <label><span>Jalur kontak</span><select id="venueDataStatus"><option value="">Semua data</option><option value="direct">Ada kontak langsung</option><option value="whatsapp">Ada WhatsApp</option><option value="instagram">Ada Instagram / DM</option><option value="official">Ada kanal resmi</option><option value="needs-verification">Belum ada kontak langsung</option></select></label>
 </div>
 <div class="venue-result-line"><strong id="venueShown">0</strong> venue tampil <span>•</span> Data riset Jabodetabek memakai JSON/CSV sebagai sumber hitung utama.</div>
...
function actions(venue,meta){
 const number=cleanNumber(meta.phone);
 const secondary=cleanNumber(meta.secondaryPhone);
 ...
 const message=encodeURIComponent(formalMessage(venue.name));
 const rows=[];
...
function contactRows(meta){
 const username=instagramUsername(meta.instagram);
 return [
  meta.phone&&`<div><span>Admin / Telepon</span><strong>${esc(meta.phoneLabel||meta.phone)}</strong></div>`,
  meta.secondaryPhone&&`<div><span>Kontak alternatif</span><strong>${esc(meta.secondaryPhoneLabel||meta.secondaryPhone)}</strong></div>`,
  ...
  meta.website&&`<div><span>Website</span><strong>${esc(new URL(meta.website,location.href).hostname.replace(/^www\./,''))}</strong></div>`
 ].filter(Boolean).join('');
}
function renderCard(venue){
 const meta=metaFor(venue),direct=hasDirect(meta),official=hasOfficial(meta),contacts=contactRows(meta);
 return `<article class="venue-card venue-card-v5 ${official?'verified':'pending'}">
  <div class="venue-thumb venue-thumb-v5"><div class="venue-court-art" aria-label="Venue ${esc(venue.name)}"><i></i><span>${esc(initials(venue.name))}</span></div><span class="venue-city">${esc(venue.city)} • ${esc(venue.province)}</span><span class="venue-data-badge ${official?'verified':'pending'}">${direct?'KONTAK ADA':official?'KANAL RESMI':'BELUM ADA KONTAK'}</span></div>
  <div class="venue-body venue-body-v5">
   <div class="venue-title-row"><div><h3>${esc(venue.name)}</h3><p class="venue-location">${esc(venue.city)}, ${esc(venue.province)}</p></div><span class="venue-source-mini">${esc(meta.verifiedAt||'Riset 14 Juli 2026')}</span></div>
   ...
   ${contacts?`<div class="venue-contact-grid">${contacts}</div>`:`<div class="venue-no-contact"><strong>Kontak publik belum ditemukan.</strong><span>Venue tetap ditampilkan karena data jumlah court atau keberadaannya telah masuk riset.</span></div>`}
   <div class="venue-actions venue-actions-v5">${actions(venue,meta)}</div>
   <div class="venue-source-row"><span>🛡️ ${esc(meta.source||'Riset venue dan Google Maps')}</span>${meta.sourceUrl?`<a href="${esc(meta.sourceUrl)}" target="_blank" rel="noopener">Buka sumber</a>`:''}</div>
  </div>
 </article>`;
}
function matches(venue,query,province,status){
 const meta=metaFor(venue);
 const research=(window.PBPI_COURT_RESEARCH||[]).find(x=>(x.display_name||x.canonical_name)===venue.name);
 const hay=[venue.name,venue.city,venue.province,venue.address,research?.venue_id,research?.district,meta.phone,meta.instagram,meta.source].join(' ').toLowerCase();
 const direct=hasDirect(meta),username=instagramUsername(meta.instagram);
 const statusOk=!status||(status==='direct'&&direct)||(status==='whatsapp'&&Boolean(meta.whatsapp||meta.phone))||(status==='instagram'&&Boolean(username))||(status==='official'&&hasOfficial(meta))||(status==='needs-verification'&&!direct);
...
function render(){
 ...
 const all=getVenues(),shown=all.filter(v=>matches(v,query,province,status));
 ...
 const metas=all.map(metaFor);
 document.getElementById('venueTotal').textContent=String(all.length);
 document.getElementById('venueAddress').textContent=String(all.filter(v=>v.address||v.searchQuery).length);
 document.getElementById('venueContact').textContent=String(metas.filter(hasDirect).length);
 document.getElementById('venueSocial').textContent=String(metas.filter(m=>instagramUsername(m.instagram)).length);
 document.getElementById('venueShown').textContent=String(shown.length);
 document.dispatchEvent(new CustomEvent('venue-directory-rendered',{detail:{total:all.length,shown:shown.length}}));
}
function install(){
 const section=document.getElementById('lapangan');if(!section)return;
 section.dataset.directoryResearchV11='1';section.innerHTML=shell();
 const provinces=[...new Set(getVenues().map(v=>v.province).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'id'));
 const select=document.getElementById('venueProvince');
 select.innerHTML='<option value="">Semua provinsi</option>'+provinces.map(p=>`<option value="${esc(p)}">${esc(p)}</option>`).join('');
 document.getElementById('venueSearch')?.addEventListener('input',render);
 select?.addEventListener('change',render);
 document.getElementById('venueDataStatus')?.addEventListener('change',render);
 document.addEventListener('click',event=>{if(event.target.closest('[data-tab="lapangan"]'))setTimeout(render,60)});
 window.renderVenueDirectoryV11=render;
 render();
}
